...
if __name__ == '__main__':
    win = ImageWin("My Window",480,640)             #Create window
    oImage = FileImage('lcastle.gif')                           #Assign image
    print(oImage.getWidth(), oImage.getHeight())            #Print image width and height
    oImage.draw(win)                                                        #Draw image on window
    myImage = oImage.copy()                                         #Make a copy and assign it

    for row in range(myImage.getHeight()):                  #Nested iteration
        for col in range(myImage.getWidth()):
             v = myImage.getPixel(col,row)                      #Get pixel and assign it
             v.red = 255 - v.red                                                #Change red of pixel
             v.green = 255 - v.green                                    #Change green of pixel
             v.blue = 255 - v.blue                                          #Chanbe blue of pixel
#             x = map(lambda x: 255-x, v)
             myImage.setPixel(col,row,v)                                #Set changed pixel
    myImage.setPosition(myImage.getWidth()+1,0)
    myImage.draw(win)                                                                   #Re-draw image
    print(win.getMouse())
